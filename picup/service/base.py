...
class Base:
    ...
    def __init__(self, scale_rate, icon, version):
        ...
        self._compress_mapping = {0: ("关闭", None, self._ui.compress_close)}
        sub_cls = CompressBase.__subclasses__()
        if sub_cls:
            ratio = int(6 / len(sub_cls))
            for index, cls in enumerate(sub_cls):
                ...
                self._ui.compress.setStretch(button_id + 1, ratio)  
        ...
        self._ui.compress.addItem(spacerItem3)
        self._ui.compress.setStretch(len(sub_cls) + 2, 1)
